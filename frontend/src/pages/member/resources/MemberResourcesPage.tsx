...
export function MemberResourcesPage() {
  ...
  async function loadResources() {
    ...
    try {
      ...
      setResources(data.resources.filter(i => i.status === "published" && i.targetRoles.includes("MEMBER")));
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="aw-stat-grid !grid-cols-2 md:!grid-cols-4">
        <div className="aw-stat-card"><p className="aw-stat-label">Available Files</p><p className="aw-stat-value">{resources.length}</p></div>
        <div className="aw-stat-card"><p className="aw-stat-label">Categories</p><p className="aw-stat-value text-[var(--aw-primary)]">{categories.length}</p></div>
      </div>
      ...
      <section className="aw-panel shadow-soft">
        <header className="aw-panel-header !bg-[var(--aw-surface)] !py-6">
          <div className="min-w-0">
             ...
             <p className="text-xs font-bold text-[var(--aw-muted)] mt-1">Official member documentation and shared media.</p>
          </div>
        ...
        <div className="p-6">
          {isLoading ? (
             <div className="py-20 text-center font-bold text-[var(--aw-muted)]">Opening vault...</div>
          ) : filtered.length === 0 ? (
             <div className="py-20 text-center flex flex-col items-center gap-4 text-[var(--aw-muted)]"><FolderOpen size={48} strokeWidth={1}/><p className="font-bold">No resources found.</p></div>
          ) : (
                  ...
                  <article key={item.id} className="aw-panel !rounded-2xl p-5 flex flex-col h-full hover:border-[var(--aw-primary)] transition-all">
                    ...
                    {item.fileId && (
                      <div className="flex gap-2 pt-4 border-t border-[var(--aw-border-soft)]">
                         <button onClick={() => window.open(getFileDownloadUrl(item.fileId as string) + "&inline=true", "_blank")} className="aw-btn aw-btn-outline !min-h-[34px] !px-3 !text-xs flex-1"><Eye size={14}/>View</button>
