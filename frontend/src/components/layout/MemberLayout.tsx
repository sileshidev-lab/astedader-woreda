...
import { BookOpen, FolderOpen, LayoutDashboard, ClipboardList, Settings, UserCircle } from "lucide-react";
import {
  AdminShellLayout,
...
const navItems: AdminShellNavItem[] = [
  { label: "Dashboard", path: "/member/dashboard", icon: LayoutDashboard, privileges: ["profile.read"] },
  { label: "Broadcasts", path: "/member/broadcasts", icon: BookOpen, privileges: ["broadcast.read"] },
  { label: "Forms", path: "/member/forms", icon: ClipboardList, privileges: ["profile.read"] },
...
export function MemberLayout() {
  const getPageHeader = useCallback((pathname: string): AdminShellHeaderInfo => {
    if (pathname.startsWith("/member/dashboard") || pathname === "/member") {
      return {
        section: "Member workspace",
        title: "Dashboard",
        description: "A summary of your profile and the modules available to your account.",
      };
    }
    ...
    if (pathname.startsWith("/member/forms")) {
      return {
        section: "Participation",
        title: "Forms",
        description: "Forms are shown here when the backend exposes a forms module.",
      };
    }
