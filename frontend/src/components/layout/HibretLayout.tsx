...
const navItems: AdminShellNavItem[] = [
  { label: "Dashboard", path: "/hibret/dashboard", icon: LayoutDashboard, privileges: ["announcement.read"] },
  { label: "Announcements", path: "/hibret/announcements", icon: Bell, privileges: ["announcement.read"] },
  { label: "Reports", path: "/hibret/reports", icon: ClipboardList, privileges: ["report.read"] },
...
export function HibretLayout() {
  const getPageHeader = useCallback((pathname: string): AdminShellHeaderInfo => {
    if (pathname.startsWith("/hibret/dashboard") || pathname === "/hibret") {
      return {
        section: "Hibret console",
        title: "Dashboard",
        description: "Overview of assigned directives, pending reports, and recent changes.",
      };
    }
    ...
    if (pathname.startsWith("/hibret/reports")) {
      return {
        section: "Reporting",
        title: "Reports",
        description: "Review your draft, submitted, and reviewed directive reports.",
      };
    }
