...
const hibret_schemas_1 = require("./hibret.schemas");
...
router.get("/", (0, requirePrivilege_1.requirePrivilege)("hibret.read"), async (_req, res) => {
    ...
    return res.json({
        hibrets: hibrets.map((hibret) => {
            const submittedReports = hibret.reports.filter((report) => Boolean(report.submittedAt));
            const reviewedReports = hibret.reports.filter((report) => Boolean(report.reviewDecision));
            const submittedAnnouncementIds = new Set(submittedReports.map((report) => report.announcementId));
            const pendingReports = hibret.announcementTargets.filter((target) => !submittedAnnouncementIds.has(target.announcementId)).length;
            const recentDirectiveTargets = [...hibret.announcementTargets]
                .sort((a, b) => new Date(b.announcement.createdAt).getTime() -
            ...
            return {
                ...
                latestSubmittedAt: submittedReports
...
router.post("/", (0, requirePrivilege_1.requirePrivilege)("hibret.create"), async (req, res) => {
    const parsed = hibret_schemas_1.createHibretSchema.safeParse(req.body);
    if (!parsed.success) {
    ...
    const hibret = await client_1.prisma.hibret.create({
        data: {
...
router.get("/:hibretId", (0, requirePrivilege_1.requirePrivilege)("hibret.read"), async (req, res) => {
    ...
    const reportByAnnouncementId = new Map(hibret.reports.map((report) => [report.announcementId, report]));
    const submittedReports = hibret.reports.filter((report) => Boolean(report.submittedAt));
    const reviewedReports = hibret.reports.filter((report) => Boolean(report.reviewDecision));
    const submittedAnnouncementIds = new Set(submittedReports.map((report) => report.announcementId));
    const pendingReports = hibret.announcementTargets.filter((target) => !submittedAnnouncementIds.has(target.announcementId)).length;
...
router.patch("/:hibretId/accounts/status", (0, requirePrivilege_1.requirePrivilege)("member_account.update"), async (req, res) => {
    const hibretId = param(req.params.hibretId);
...
router.patch("/:hibretId/accounts/:userId/status", (0, requirePrivilege_1.requirePrivilege)("member_account.update"), async (req, res) => {
    const hibretId = param(req.params.hibretId);
...
router.post("/:hibretId/families", (0, requirePrivilege_1.requirePrivilege)("family.create"), async (req, res) => {
    const hibretId = param(req.params.hibretId);
    ...
    const existing = await client_1.prisma.family.findFirst({
        where: {
            hibretId,
            name: {
                equals: name,
    ...
    const family = await client_1.prisma.family.create({
        data: {
            hibretId,
            name,
            contactName,
...
router.patch("/:hibretId/families/:familyId", (0, requirePrivilege_1.requirePrivilege)("family.update"), async (req, res) => {
    const hibretId = param(req.params.hibretId);
    ...
    const updated = await client_1.prisma.family.update({
        where: { id: familyId },
        data: {
            ...(name !== undefined ? { name } : {}),
...
router.delete("/:hibretId/families/:familyId", (0, requirePrivilege_1.requirePrivilege)("family.delete"), async (req, res) => {
    const hibretId = param(req.params.hibretId);
    const familyId = param(req.params.familyId);
    const family = await client_1.prisma.family.findUnique({
        where: { id: familyId },
    ...
    await client_1.prisma.family.delete({
        where: { id: familyId },
    });
    return res.json({ message: "Family deleted." });
});
router.post("/:hibretId/families/:familyId/members", (0, requirePrivilege_1.requirePrivilege)("family.update"), async (req, res) => {
    const hibretId = param(req.params.hibretId);
    const familyId = param(req.params.familyId);
    ...
    await client_1.prisma.member.updateMany({
        where: {
            id: {
                in: memberIds,
            },
...
router.post("/:hibretId/families/unassign-members", (0, requirePrivilege_1.requirePrivilege)("family.update"), async (req, res) => {
    const hibretId = param(req.params.hibretId);
    const memberIds = Array.isArray(req.body.memberIds) ? req.body.memberIds.map(String) : [];
    if (memberIds.length === 0) {
        return res.status(400).json({ message: "Select at least one member." });
...
router.patch("/:hibretId", (0, requirePrivilege_1.requirePrivilege)("hibret.update"), async (req, res) => {
    const hibretId = param(req.params.hibretId);
    const parsed = hibret_schemas_1.updateHibretSchema.safeParse(req.body);
    if (!parsed.success) {
        return res.status(400).json({
            message: "Invalid Hibret data",
            errors: parsed.error.flatten(),
    ...
    const hibret = await client_1.prisma.hibret.update({
        where: { id: hibretId },
        data: {
            name: parsed.data.name,
