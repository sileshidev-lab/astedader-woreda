...
import { getPdfLang, getPdfTranslation, type PdfTranslation } from "./pdf-translations";
...
function safeAsciiDownloadName(value: string, fallback = "export") {
...
function setAttachmentHeader(res: any, filename: string) {
...
function isWoredaUser(req: any) {
  return req.user?.role === "WOREDA_ADMIN";
}

function requireWoreda(req: any, res: any) {
  if (!isWoredaUser(req)) {
...
function requireExportPrivilege(req: any, res: any, privilege = "report.export") {
...
function buildAttendanceCsv(records: any[]) {
  const rows = [
    [
      "Member Full Name",
      "Member Code",
...
async function getAttendanceForReport(report: any) {
...
async function getAttendanceExportContext(announcementId: string, hibretId: string) {
  ...
  if (!announcement || !hibret) return null;

  const recordByMemberId = new Map(records.map((record: any) => [record.memberId, record]));
...
function buildAttendanceExportCsv(context: any) {
  const headerRows = [
...
async function buildAttendanceWorkbookBuffer(context: any) {
...
function addReportAttachmentsToArchive(archive: any, report: any, reportFolder = "") {
...
async function sendReportPackageZip(res: any, report: any) {
...
async function getReportForExport(reportId: string) {
  return prisma.hibretReport.findUnique({
    where: { id: reportId },
    include: {
      announcement: true,
      hibret: true,
...
router.get("/announcements/export.csv", authMiddleware, async (req, res) => {
  if (!requireWoreda(req, res)) return;
  if (!requireExportPrivilege(req, res, "announcement.export")) return;
  ...
  const rows = [
    ...
    ...announcements.map((announcement) => [
      ...
      String(((announcement as any).targets?.length ?? (announcement as any).hibrets?.length ?? 0)),
...
router.get("/announcements/:announcementId/reports.csv", authMiddleware, async (req, res) => {
  if (!requireWoreda(req, res)) return;
  if (!requireExportPrivilege(req, res, "announcement.export")) return;
...
router.get("/announcements/:announcementId/export.zip", authMiddleware, async (req, res) => {
  if (!requireWoreda(req, res)) return;
  if (!requireExportPrivilege(req, res, "announcement.export")) return;
  ...
  const t = getPdfTranslation(req.query.lang);
  const lang = getPdfLang(req.query.lang);
  const reportHtml = formalReportHtml(
    t.directiveReportSummary,
    `${t.directive}: ${announcement.title}`,
...
router.get("/woreda/reports/:reportId/export.html", authMiddleware, async (req, res) => {
  if (!requireWoreda(req, res)) return;
  if (!requireExportPrivilege(req, res, "report.export")) return;
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="${safeName(report.title)}-report.html"`
  );
...
router.get("/woreda/reports/:reportId/package.zip", authMiddleware, async (req, res) => {
  if (!requireWoreda(req, res)) return;
  if (!requireExportPrivilege(req, res, "report.export")) return;
...
router.get("/hibret/reports/:reportId/export.html", authMiddleware, async (req, res) => {
  const report = await getReportForExport(String(req.params.reportId));
...
router.get("/hibret/reports/:reportId/package.zip", authMiddleware, async (req, res) => {
  const report = await getReportForExport(String(req.params.reportId));
...
router.get("/woreda/reports/:reportId/attachments.zip", authMiddleware, async (req, res) => {
  if (!requireWoreda(req, res)) return;
  if (!requireExportPrivilege(req, res, "report.export")) return;
...
router.get("/woreda/reports/:reportId/export.zip", authMiddleware, async (req, res) => {
  if (!requireWoreda(req, res)) return;
  if (!requireExportPrivilege(req, res, "report.export")) return;
  ...
  const t = getPdfTranslation(req.query.lang || "en");
  const reportPdf = await htmlToPdfBuffer(buildReportHtml(report), t);
...
// ===== FORMAL BRANDED ANNOUNCEMENT HTML PDF EXPORTS =====

type AnnouncementMetricsRow = {
  hibretName: string;
  submitted: boolean;
...
function getPdfLogoPath() {
  return findExistingPath([
    path.resolve(process.cwd(), "../frontend/public/Prosperity_Party_logo.png"),
    path.resolve(process.cwd(), "../frontend/public/prosperity-party-logo.png"),
    path.resolve(process.cwd(), "public/Prosperity_Party_logo.png"),
    path.resolve(process.cwd(), "uploads/Prosperity_Party_logo.png"),
  ]);
}
...
function fontToDataUri(filePath?: string) {
  if (!filePath || !fs.existsSync(filePath)) return "";
  const ext = path.extname(filePath).toLowerCase();
...
function escapePdfHtml(value?: string | number | Date | null) {
  return String(value ?? "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
function normalizeDecision(value: string | null | undefined, t: PdfTranslation) {
  if (!value) return t.pending;
  if (value === "approved") return t.approved;
  if (value === "rejected") return t.rejected;
  if (value === "changes_requested") return t.changesRequested;
  return String(value).replace(/_/g, " ");
}

function isReportMedia(file: any) {
  const mimeType = String(file?.mimeType || "").toLowerCase();
  return mimeType.startsWith("image/") || mimeType.startsWith("video/");
}

function qualityForReport(report: any, announcement: any, attendance: { completionRate: number }) {
  if (!report?.submittedAt) {
    return { score: 0, label: "Not submitted" };
  }

  const attachments = report.attachments ?? [];
  const mediaCount = attachments.filter((attachment: any) => isReportMedia(attachment.file)).length;
  ...
  let score = 0;
  if (String(report.title || "").trim()) score += 10;
  if (summaryText.length >= 20) score += 15;
  if (bodyText.length >= 100) score += 25;
  if (attachments.length > 0) score += 15;
  if (mediaCount > 0) score += 10;
  if (onTime) score += 15;
  if (!announcement.attendanceRequired || attendance.completionRate >= 100) score += 10;
...
async function getAnnouncementsForFormalExport(announcementId?: string) {
...
function buildAnnouncementMetrics(announcement: any, t: PdfTranslation) {
  const targets = announcement.targets ?? [];
  const reports = announcement.reports ?? [];
  const submittedReports = reports.filter((report: any) => Boolean(report.submittedAt));
  const reportByHibretId = new Map<string, any>(
    reports.filter((report: any) => Boolean(report.submittedAt)).map((report: any) => [report.hibretId, report])
  );
  ...
  const hibretRows: AnnouncementMetricsRow[] = targets.map((target: any) => {
    const hibret = target.hibret;
    const report = reportByHibretId.get(target.hibretId);
    const records = attendanceRecords.filter((record: any) => record.hibretId === target.hibretId);
    ...
    const excused = records.filter((record: any) => record.status === "excused").length;
    ...
    if (submittedAt && isOnTime) onTime += 1;
    if (submittedAt && !isOnTime) late += 1;
    ...
    return {
      ...
      media: report?.attachments?.filter((attachment: any) => isReportMedia(attachment.file)).length ?? 0,
    };
  });
...
function rowsHtml(rows: string[][], emptyColspan: number, t: PdfTranslation) {
  if (!rows.length) {
...
function tableHtml(headers: string[], rows: string[][], t: PdfTranslation) {
  return `
    <table>
      <thead>
...
function donutChartHtml(title: string, segments: ChartSegment[]) {
...
function barChartHtml(title: string, rows: BarChartRow[], suffix = "") {
...
function executiveChartsHtml(announcement: any, metrics: ReturnType<typeof buildAnnouncementMetrics>, t: PdfTranslation) {
  const reviewCounts = metrics.hibretRows.reduce(
    (acc, row) => {
      if (!row.submitted) {
        acc.pending += 1;
        return acc;
...
function reportDetailSectionHtml(report: any, announcement: any, t: PdfTranslation) {
  const attachments = report.attachments ?? [];
  const reviews = report.reviews ?? [];
  const attendanceRecords = (announcement.attendanceRecords ?? []).filter(
    (record: any) => record.hibretId === report.hibretId
  );
  ...
  return `
    <section class="report-detail-block">
      ...
      ${tableHtml(
        [t.fileName, t.fileType, t.fileSize],
...
function submittedReportsDetailHtml(announcement: any, t: PdfTranslation) {
  const reports = (announcement.reports ?? []).filter((report: any) => Boolean(report.submittedAt));
  ...
  return reports
    .map((report: any) => reportDetailSectionHtml(report, announcement, t))
    .join("");
}


function announcementSectionHtml(announcement: any, t: PdfTranslation, detailed = true) {
  ...
  return `
    <section class="report-section page-break-before">
      ...
      ${tableHtml(
        [t.hibrets, t.families, t.members, t.submittedAt, t.review, t.speed],
        metrics.reportedRows.map((row) => [
          row.hibretName,
      ...
      ${tableHtml(
        [t.hibrets, t.families, t.members, t.report],
        metrics.missingRows.map((row) => [
          row.hibretName,
          String(row.familyTotal),
          String(row.memberTotal),
          t.notReported,
      ...
      ${
        detailed
          ? `
            ...
            ${tableHtml(
              [t.hibrets, t.families, t.members, t.marked, t.present, t.absent, t.excused, t.unmarked, t.completion],
              metrics.hibretRows.map((row) => [
                row.hibretName,
                String(row.familyTotal),
                String(row.memberTotal),
                String(row.marked),
...
function formalReportHtml(title: string, subtitle: string, body: string, t: PdfTranslation) {
...
async function htmlToPdfBuffer(html: string, t: PdfTranslation) {
...
function sendPdfBuffer(res: any, filename: string, buffer: Buffer | Uint8Array) {
  res.setHeader("Content-Type", "application/pdf");
  setAttachmentHeader(res, `${filename}.pdf`);
  return res.send(Buffer.from(buffer));
}

router.get("/announcements/export.pdf", authMiddleware, async (req, res) => {
  if (!requireWoreda(req, res)) return;
  if (!requireExportPrivilege(req, res, "announcement.export")) return;
  ...
  const aggregate = announcements.reduce(
    (acc: any, announcement: any) => {
      const metrics = buildAnnouncementMetrics(announcement, t);
      acc.announcements += 1;
  ...
  const summaryRows = announcements.map((announcement: any) => {
    const metrics = buildAnnouncementMetrics(announcement, t);
    return [
      announcement.title,
  ...
  const html = formalReportHtml(
    t.announcementPerformanceReport,
    t.announcementPerformanceSubtitle,
    `
      <section class="report-section">
        ...
        ${tableHtml(
          [t.announcement, t.type, t.deadline, t.hibrets, t.reported, t.missing],
          summaryRows,
          t
        )}
      </section>

      ${announcements.map((announcement: any) => announcementSectionHtml(announcement, t, true)).join("")}
    `,
    t
  );
...
router.get("/announcements/:announcementId/reports.pdf", authMiddleware, async (req, res) => {
  if (!requireWoreda(req, res)) return;
  if (!requireExportPrivilege(req, res, "announcement.export")) return;
...
router.get("/woreda/announcements/:announcementId/hibrets/:hibretId/attendance.csv", authMiddleware, async (req, res) => {
  if (!requireWoreda(req, res)) return;
  if (!requireExportPrivilege(req, res, "attendance.export")) return;

  const context = await getAttendanceExportContext(
...
router.get("/woreda/announcements/:announcementId/hibrets/:hibretId/attendance.xlsx", authMiddleware, async (req, res) => {
  if (!requireWoreda(req, res)) return;
  if (!requireExportPrivilege(req, res, "attendance.export")) return;
