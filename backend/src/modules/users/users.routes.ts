...
function canManageScope(req: any, user: any) {
  if (req.user?.role === "WOREDA_ADMIN") return true;
  if (req.user?.role === "HIBRET_ADMIN" && user.hibretId === req.user?.hibretId) return true;
  return false;
}
...
router.get("/", requirePrivilege("member_account.read"), async (req: any, res) => {
...
router.patch("/:userId/status", requirePrivilege("member_account.update"), async (req: any, res) => {
...
router.patch("/bulk/status", requirePrivilege("member_account.update"), async (req: any, res) => {
  const userIds = Array.isArray(req.body.userIds) ? req.body.userIds.map(String) : [];
  const status = String(req.body.status || "").trim();

  if (!userIds.length) return res.status(400).json({ message: "Select at least one user." });
  if (!["ACTIVE", "DISABLED", "PENDING_SETUP"].includes(status)) {
    return res.status(400).json({ message: "Invalid account status." });
  }
