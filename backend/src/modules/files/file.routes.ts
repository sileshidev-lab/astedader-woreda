...
import { announcementUpload, broadcastUpload, resourceUpload, reportUpload, memberPhotoUpload } from "./upload";
...
function resolveStoredFilePath(storedPath: string) {
  ...
  return candidates.find((candidate) => fs.existsSync(candidate)) || path.resolve(process.cwd(), storedPath);
}

function verifyRequestToken(req: any, res: any) {
...
router.post(
  "/upload/announcement",
  authMiddleware,
  requirePrivilege("announcement.create"),
...
router.post(
  "/upload/broadcast",
  authMiddleware,
  requirePrivilege("broadcast.create"),
  broadcastUpload.single("file"),
  async (req, res) => {
    if (!req.file) {
      return res.status(400).json({ message: "File is required" });
    }
...
router.post(
  "/upload/resource",
  authMiddleware,
  requirePrivilege("resource.create"),
...
router.post(
  "/upload/member",
  authMiddleware,
  requirePrivilege("member.update"),
  memberPhotoUpload.single("file"),
  async (req, res) => {
    if (!req.file) {
      return res.status(400).json({ message: "File is required." });
    }
...
router.get("/:fileId/preview", async (req, res) => {
  if (!verifyRequestToken(req, res)) return;
...
router.get("/:fileId/download", async (req, res) => {
  ...
  if (inline) {
    ...
    res.setHeader(
      "Content-Disposition",
      contentDispositionValue("inline", file.originalName)
    );
